...
function partA(typeOfData: string): number {
    ...
    for (let i = 0; i < input.length; i = i + 4) {
        let minNum = +input[i];
        let maxNum = +input[i + 1];
        let char = input[i + 2];
        let password = input[i + 3];
        let pieces = password.split(char);
        let charCount = pieces.length - 1;

        if ((charCount >= minNum) && (charCount <= maxNum)) {
            count++;
        }
    }
...
function partB(typeOfData: string): number {
    ...
    for (let i = 0; i < input.length; i = i + 4) {
        ...
        let charAtPosMin = password.charAt(minNum - 1);
        let charAtPosMax = password.charAt(maxNum - 1);
        if ((char == charAtPosMin) && (char != charAtPosMax)) {
            count++;
        } else if ((char != charAtPosMin) && (char == charAtPosMax)) {
            count++;
        }
    }
